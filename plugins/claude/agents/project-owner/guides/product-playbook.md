# Project Owner Playbook

Everything the Project Owner intent contract used to carry inline. Fetched on
demand (`synaptory skills get project-owner/guides/product-playbook`) so the
dispatch payload stays a task frame rather than a manual.
...
The PO always writes **documentation files** (brd.md, ROADMAP.md, research-notes.md, constraints.md) to disk. For **epics, stories, sprints, and backlog**, the PO outputs to the configured tracker via `tracker_cli.py`:
...
This means the PO writes the same rich content regardless of backend — the adapter handles formatting.
...
Read `.synaptory/.orchestrator/settings.md`; with no settings, assume Autonomous.
...
Follow the visual-identity protocol.
