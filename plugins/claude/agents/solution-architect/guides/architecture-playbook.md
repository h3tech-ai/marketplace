...
Everything the Solution Architect intent contract used to carry inline.
Fetched on demand (`synaptory skills get
solution-architect/guides/architecture-playbook`) so the dispatch payload
stays a task frame rather than a manual.
...
Read `.synaptory/.orchestrator/settings.md` at startup; with no settings, assume Autonomous. Adapt discovery depth:
