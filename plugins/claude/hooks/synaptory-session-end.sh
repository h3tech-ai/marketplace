...
# No directory-existence guard on the way in. It used to test the flat and
# multi-spec dirs, and an SPQ clone has NEITHER, so the whole ship loop was
# skipped before any glob ran (#326). Re-listing the layouts here would just
# recreate that bug the next time a layout is added: `synaptory_receipt_files`
# already returns nothing when there is nothing, and the loop is then a no-op.
if [[ -n "$cli" ]] && [[ -x "$cli" ]]; then
  _tu="${_HOOK_ROOT}/hooks/lib/transcript_usage.py"
  while IFS= read -r f; do
    [ -f "$f" ] || continue
    sentinel="${f}.shipped"
    if [ ! -e "$sentinel" ] || [ "$f" -nt "$sentinel" ]; then
      # Canonicalize SDK token_usage names (input_tokens → input) so
      # SessionEnd-shipped Cursor receipts still hit /cost.
      if [ -f "$_tu" ] && [ -n "${_py:-}" ]; then
        "$_py" "$_tu" "$f" >/dev/null 2>&1 || true
      fi
...
RECEIPT_COUNT=$(synaptory_receipt_count "$_orch")
