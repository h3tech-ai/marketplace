...
done < <(synaptory_receipt_files "$_orch")

# Backfill / canonicalize token_usage before shipping. Subagents can't see
# their own final Usage — that lives only in the parent's result message —
# so receipts written by the subagent ship with no token counts and /cost
# rolls them up at $0. Cursor agents also copy SDK names (input_tokens)
# into token_usage; patch_receipt rewrites those onto Cost's canonical
# keys even when there is no parent transcript. Best-effort: a parse miss
# leaves the receipt untouched and the validator's existing warning still
# fires. See transcript_usage.py.
if [ "${#RECEIPTS_TO_SHIP[@]}" -gt 0 ] \
   && [ -f "$HOOK_LIB_DIR/transcript_usage.py" ] && [ -n "${_py:-}" ]; then
  for receipt in "${RECEIPTS_TO_SHIP[@]}"; do
    "$_py" "$HOOK_LIB_DIR/transcript_usage.py" "$receipt" \
...
if [ -f "$HOOK_LIB_DIR/dispatched_receipts.py" ] && [ -n "${_py:-}" ]; then
  ...
  if [ "$SELECTION_MODE" != "unbound" ]; then
    ...
    # Newest first from the helper; the newest anchors the missing check.
...
if [ -f "$LOGGER" ] && [ -n "${_py:-}" ]; then
  "$_py" "$LOGGER" "$CLAUDE_PROJECT_DIR" receipt_selection \
    "mode=$SELECTION_MODE" "count=${#RECEIPTS_TO_VALIDATE[@]}" >/dev/null 2>&1 || true
fi
...
_validate_one_receipt() {
  _receipt="$1"
  VALIDATION_OUTPUT=$("$_py" "$HOOK_LIB_DIR/receipt_validator.py" "$_receipt" "$CLAUDE_PROJECT_DIR" 2>/dev/null)
  VALIDATION_EXIT=$?
...
" 2>/dev/null
  ...
  if [ -f "$HOOK_LIB_DIR/verification_runner.py" ]; then
    VERIFY_OUTPUT=$("$_py" "$HOOK_LIB_DIR/verification_runner.py" "$_receipt" "$CLAUDE_PROJECT_DIR" 2>/dev/null)
    VERIFY_EXIT=$?
...
if [ -f "$HOOK_LIB_DIR/receipt_validator.py" ] && [ -n "${_py:-}" ]; then
  for RECEIPT_TO_CHECK in "${RECEIPTS_TO_VALIDATE[@]}"; do
    _validate_one_receipt "$RECEIPT_TO_CHECK"
  done
  # All checks passed
  exit 0
else
