...
5. **Write `.synaptory.yaml`** from template `Bash("synaptory skills get templates/synaptory.yaml.tmpl")`, filling in detected values. Resolve `{{PRODUCT_VERSION}}` by running `python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/product_version.py"` from the executing plugin and use its exact output as the top-level `version`. If release metadata is unavailable, report that error before writing config. Continue with analysis.
