...
- **Never spell a receipt path.** Resolve it, so the path you name is the path the gate reads:
  ```bash
  python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/advance_kernel.py" bind_receipt "$(pwd)" {UNIT_OR_PSEUDO_ID} {ROLE_ABBREV}
  ```
...
| Situation | File |
|---|---|
| No open Cycle, or the last one closed and the next is being sized | `Read("${CLAUDE_PLUGIN_ROOT}/skills/synaptory/spq/commit.md")` |
| Work Units are admitted and running | the per-unit loop below |
| A Work Unit waits on something another Cycle has not published | `Read("${CLAUDE_PLUGIN_ROOT}/skills/synaptory/spq/sync.md")` |
| Every admitted unit is done or cut | `Read("${CLAUDE_PLUGIN_ROOT}/skills/synaptory/spq/checkpoint.md")` |
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/advance_kernel.py" begin_dispatch "$(pwd)" {UNIT_ID} --role {ROLE_ABBREV}
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/advance_kernel.py" advance "$(pwd)" {UNIT_ID} {TO_STATE} --receipt-path {RECEIPT_PATH}
```
...
**A dispatch output carries `parallel`, and you MUST act on it.** `next_action` names one unit in `story_id` and, alongside it, every OTHER admitted unit that is dispatchable right now with a declared `path_scope` pairwise-disjoint from the rest of the batch. **When `parallel.eligible` is true, dispatch every member of `parallel.batch` concurrently** -- `batch[0]` is the serial action, and each further entry carries its own `story_id` and `role`. Authorize each one through `begin_dispatch` exactly as you would a serial dispatch; they are the same units the kernel authorizes today. `parallel.reason` names what was held back and why (an intersecting scope, an absent declaration, the ceiling).

Dispatching only `story_id` when the batch names more is the measured failure, not a conservative choice: `#807` recorded seven mutually disjoint units at a mean concurrency of 0.95, with exactly one unit running for 44% of the Cycle and nothing at all for 41%.

**`trunk_drift` on a dispatch means the trunk moved under this Cycle.** It appears only when it did. Work merged to the trunk while a Cycle is open is inherited by an ordinary merge and is **not** the unit's own change -- state `trunk_drift.notice` verbatim in the dispatch prompt, and treat a regression that appears under it as possibly belonging to the trunk until you have checked. Do not attribute it to the unit under review by default.
