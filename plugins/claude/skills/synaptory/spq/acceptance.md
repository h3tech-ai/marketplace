...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" record_handover "$(pwd)" \
  --recorded-by "<name>" --codebase "<rev>" \
  --documentation "<doc ref>" --operating-knowledge "<runbook ref>"

python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" record_commitment "$(pwd)" \
  --description "<what is still owed>" --recorded-by "<name>"

python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" discharge_commitment "$(pwd)" \
