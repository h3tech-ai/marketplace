...
`plugin-claude/rules/*.md` when the CLI is absent, and assembles them into a
...
the plaintext `cat plugin-claude/rules/*.md` loop with a CP-first fetch.
"""
...
def test_disk_fallback_when_cli_missing(
    ...
    """No CLI on PATH → bodies come from on-disk plugin-claude/rules/*.md."""
