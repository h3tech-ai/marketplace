...
```bash
MCP `spq_lifecycle` {"operation": "hydrate_cycle", "goal": "<the Cycle goal from Commit>", "work_units": "<the filtered backlog as JSON: [{\"id\":\u2026,\"title\":\u2026,\"depends_on\":[\u2026],\"file_scope\":[\u2026]}]>"}
```
...
| `action` | What you do |
|---|---|
| `dispatch_se` | Dispatch the software-engineer for `story_id` (queued means run `MCP `begin_lifecycle_dispatch` {"role": "se", "story_id": "{story_id}"}` first, which authorizes the dispatch against `next_action` and returns the canonical receipt path to write; in_progress means resume). If `receipt_present: true`, skip the dispatch and run `MCP `advance` {"story_id": "{story_id}", "to_state": "{transition_to}"}` instead, because the agent already finished and only the transition is missing. If the output carries `parallel.eligible: true`, dispatch the whole batch via the worktree procedure in `skills/_shared/backends/claude.md` § Parallel story dispatch. |
...
| `promote_story` | Run `MCP `advance` {"story_id": "{story_id}", "to_state": "done"}`. The kernel validates the CR receipt and fires DoD, redirecting to `blocked` if a conditional gate is unsatisfied. |
...
4. **DoD evaluation**: the kernel invokes `evaluate_story_dod` on the `→ done` edge. To re-run explicitly:
   ```bash
   MCP `spq_lifecycle` {"operation": "evaluate_dod", "story_id": "{WORK_UNIT_ID}"}
   ```
...
2. In the integration clone, the human merges that branch into the manifest's `integration_ref` locally. Before pushing the result, run:
   ```bash
   python3 "${PLUGIN_ROOT}/hooks/lib/sync_barrier.py" \
...
3. After a green result, the human pushes that exact integration HEAD. In the producer clone, fetch the sealed `integration_ref` and publish the verifiable event with the returned attestation:
   ```bash
   MCP `spq_publish_event` {"condition": "integrated", "commit_sha": "{WORK_SHA}", "cycle_id": "{CYCLE_ID}", "manifest_hash": "{MANIFEST_HASH}", "evaluation": {INCREMENTAL_ATTESTATION_JSON}, "work_unit_id": "{WORK_UNIT_ID}"}
   ```
...
```bash
MCP `spq_lifecycle` {"operation": "declare_ready", "workstream": "{WORKSTREAM_ID}", "approved_by": "{LEAD_EMAIL}", "cycle": "{CYCLE_N}"}
```
...
```bash
MCP `spq_lifecycle` {"operation": "enter_sync"}
```
