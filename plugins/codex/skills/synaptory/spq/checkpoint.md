...
```bash
MCP `spq_cut_work_unit` {"reason": "<why this unit cannot make this Cycle>", "cut_by": "<the Engineering Lead's identity>", "cycle_id": "{CYCLE_ID}", "manifest_hash": "{DECLARATION_HASH}", "work_unit_id": "{UNIT_ID}"}
```
...
python3 "${PLUGIN_ROOT}/hooks/lib/advance_kernel.py" bind_receipt "$(pwd)" CHECKPOINT-{N} tw
```
...
> **MANDATORY: Spawn this agent via the `Agent()` tool.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost`. The dispatch looks like `Agent(subagent_type="synaptory:technical-writer", description="TW Cycle report", prompt=<self-contained prompt per the wrapper>)` -- see `${PLUGIN_ROOT}/skills/_shared/backends/${TW_BACKEND}.md`. Pass the resolved absolute path from `bind_receipt` into the prompt; do not pass the literal variable.
...
```bash
${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py run_barrier "$PWD"
```
...
```bash
MCP `spq_lifecycle` {"operation": "close_cycle", "principal": "<principal>", "rationale": "<why>"}
```
...
| Next | Command | When |
|---|---|---|
| another Cycle | `Read("${PLUGIN_ROOT}/skills/synaptory/spq/commit.md")` | delivery continues. `CYCLE -> CYCLE` is a legal self-transition -- closing one Cycle and opening the next is delivery continuing, not a stage change |
