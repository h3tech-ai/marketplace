...
```bash
MCP `spq_manifest_validate` {"cycle_id": "{CHILD_CYCLE_ID}"}
```
...
```bash
MCP `spq_lifecycle` {"operation": "open_coordination_cycle", "release_goal": "{RELEASE_GOAL}", "children": {CHILDREN_JSON}, "dependency_edges": {EDGES_JSON}}
```
...
```bash
MCP `spq_publish_cross_cycle_event` {"cycle_id": "{PRODUCER_CYCLE_ID}", "condition": "{CONDITION}", "output": {"kind":"contract","id":"{OUTPUT_ID}","digest":"{DIGEST}"}, "commit_sha": "{COMMIT_SHA}"}
```
...
```bash
MCP `spq_refresh_coordination`
MCP `spq_dependency_ledger`
```
...
```bash
MCP `spq_lifecycle` {"operation": "revise_coordination_manifest", "drop_child": "{CYCLE_ID}", "reason": "{WHY}"}
```
...
```bash
MCP `spq_lifecycle` {"operation": "clear_release", "cleared_by": "{UPN}"}
```
