...
OTEL_WRITER="${PLUGIN_ROOT}/hooks/lib/otel_writer.py"
...
if [ "${#RECEIPTS_TO_SHIP[@]}" -gt 0 ]; then
  cli=$("${PLUGIN_ROOT:-$(cd "$(dirname "$0")/.." && pwd)}/hooks/_resolve-cli.sh" 2>/dev/null || true)
  if [[ -n "$cli" ]] && [[ -x "$cli" ]]; then
    for receipt in "${RECEIPTS_TO_SHIP[@]}"; do
...
" 2>/dev/null)
    ...
    if [ "$AUTONOMOUS" = "true" ]; then
      ...
      echo "$VALIDATION_OUTPUT" | SYNAPTORY_HOOK_LIB="${PLUGIN_ROOT}/hooks/lib" "$_py" -c "
import sys, os, json
