...
>
> This restriction is declared in frontmatter (`allowed-tools: Read, Grep, Glob`), but when this skill is invoked as a subagent via `Task()` the host may not honor plugin frontmatter. Therefore the restriction is also enforced here in the prompt. Treat it as a hard constitutional constraint.
>
...
2. Story list — run `python3 ${PLUGIN_ROOT}/skills/_shared/scripts/tracker/tracker_cli.py --project-dir . get-backlog` for requirement overview
