...
If a mode file is specified, load it with: `Read("${PLUGIN_ROOT}/roles/software-engineer/modes/{mode}.md")` and follow its instructions entirely. Stop reading this file after the mode dispatch.
...
Read("${PLUGIN_ROOT}/roles/software-engineer/tech-packs/{detected}.md")
```
...
Read(`.synaptory/.protocols/coverage-ratchet.md`) if the file exists
