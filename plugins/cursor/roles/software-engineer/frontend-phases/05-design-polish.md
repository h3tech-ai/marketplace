...
IF [ -n "$design_ref" ] AND [ -f "$design_ref" ]:
  ...
  IF [ -n "$approved_at" ]:
    ...
    - SKIP the Style Selection block below (no AskQuestion, no auto-select)
...
The FIRST action in this phase is to ask the user which visual style they want. Use AskQuestion IMMEDIATELY:
...
**Design review (mode-aware):** Autonomous — proceed to Phase 6, report design summary. Controlled — present full design system comparison via AskQuestion.
