...
Shell("synaptory skills get design-assets/ux-psychology")
```
...
Bash("python3 ${PLUGIN_ROOT}/skills/_shared/scripts/design_assets/scripts/search.py '[domain] [product-type]' --design-system")

# Search specific design domains
Bash("python3 ${PLUGIN_ROOT}/skills/_shared/scripts/design_assets/scripts/search.py '[query]' --domain style")
Bash("python3 ${PLUGIN_ROOT}/skills/_shared/scripts/design_assets/scripts/search.py '[query]' --domain color")
Bash("python3 ${PLUGIN_ROOT}/skills/_shared/scripts/design_assets/scripts/search.py '[query]' --domain typography")

# Persist design system to project
Bash("python3 ${PLUGIN_ROOT}/skills/_shared/scripts/design_assets/scripts/search.py '[domain] [product-type]' --design-system --persist")
```
...
Shell("synaptory skills get design-assets/animation-guide")
# Visual effects — glassmorphism, shadows, gradients
Shell("synaptory skills get design-assets/visual-effects")
```
...
Task(prompt="Build auth pages (login, register, forgot-password). Use components from frontend/app/components/. Write to frontend/app/pages/auth/.", ...)
Task(prompt="Build dashboard pages (overview, analytics, activity). Use components from frontend/app/components/. Write to frontend/app/pages/dashboard/.", ...)
Task(prompt="Build settings pages (profile, billing, team, integrations). Use components from frontend/app/components/. Write to frontend/app/pages/settings/.", ...)
```
...
```python
# Run UX audit
Bash("python3 ${PLUGIN_ROOT}/skills/_shared/scripts/design_assets/scripts/ux_audit.py frontend/app/")
