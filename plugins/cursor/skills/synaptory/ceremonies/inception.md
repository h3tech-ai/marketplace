...
-->"""
    ...
    Bash(f'printf "%s" \'{claude_section}\' | python3 "${{PLUGIN_ROOT}}/hooks/lib/update_claude_md.pyc" "${{CLAUDE_PROJECT_DIR}}"')
```
...
Follow the Design Grooming Protocol at `${PLUGIN_ROOT}/skills/_shared/protocols/design-grooming.md`.
...
`${PLUGIN_ROOT}/skills/_shared/design-assets/` (component-patterns, color-palettes,
...
`completed_at` (see `${PLUGIN_ROOT}/skills/_shared/protocols/receipt-protocol.md`).
