...
```
python3 "${PLUGIN_ROOT}/hooks/lib/story_pipeline.py" list_stories "$(pwd)" queued
python3 "${PLUGIN_ROOT}/hooks/lib/story_pipeline.py" list_stories "$(pwd)" in_progress
python3 "${PLUGIN_ROOT}/hooks/lib/story_pipeline.py" list_stories "$(pwd)" testing
python3 "${PLUGIN_ROOT}/hooks/lib/story_pipeline.py" list_stories "$(pwd)" reviewing
python3 "${PLUGIN_ROOT}/hooks/lib/story_pipeline.py" list_stories "$(pwd)" blocked
```
...
For each incomplete story:
  1. Update tracker status to TO_DO (reset for next sprint):
     python3 "${PLUGIN_ROOT}/skills/_shared/scripts/tracker/tracker_cli.py" \
...
```
python3 "${PLUGIN_ROOT}/hooks/lib/scrum_state_machine.py" close_sprint "$(pwd)" \
