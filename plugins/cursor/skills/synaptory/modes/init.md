...
4. Generate `.synaptory.yaml` from the template at `${PLUGIN_ROOT}/skills/_shared/templates/synaptory.yaml.tmpl`, applying the confirmed answers.
...
# The init logic reads from: ${PLUGIN_ROOT}/skills/_shared/templates/synaptory.yaml.tmpl
...
Confirm each field with `AskQuestion`. Follow the pattern used by
`${PLUGIN_ROOT}/skills/_shared/protocols/socratic-gate.md`: the detected value is
...
if not story_exists:
    ...
    builtin_dir = f"${{PLUGIN_ROOT}}/skills/_shared/templates/tracker"
...
```
python3 ${PLUGIN_ROOT}/skills/_shared/scripts/tracker/tracker_cli.py --project-dir . <cmd>
```
...
**At the start of every session, ask the user how they'd like to work** using AskQuestion:
...
This README is updated automatically as synaptory delivers."""
    Bash(f'printf "%s" \'{readme_section}\' | python3 "${{PLUGIN_ROOT}}/hooks/lib/update_claude_md.py" "${{CLAUDE_PROJECT_DIR}}" --file README.md')
```
