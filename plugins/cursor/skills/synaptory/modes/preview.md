...
| **Interactive** | No — returns results, orchestrator decides | Yes — presents AskQuestion |
...
"verification_commands": [
  {"command": "python3 ${PLUGIN_ROOT}/hooks/lib/preview_asset_check.py http://localhost:{port} /route-a /route-b", "exit_code": 0, "summary": "All routes render and every referenced asset returns 200"},
...
```json
{
  ...
  "verification_commands": [
    {"command": "python3 ${PLUGIN_ROOT}/hooks/lib/preview_asset_check.py http://localhost:{port}", "exit_code": 0, "summary": "Root route renders with all assets 200"}
  ]
}
```
