...
```bash
python3 "${PLUGIN_ROOT}/skills/_shared/scripts/security_scan.py" . --format json
```
...
**1 gate.** Present the findings grouped by severity with the counts, then triage with `AskQuestion`:
...
If the CE ran inline rather than through `Task()` (no SubagentStop, so no receipt ship loop), the orchestrator MUST write that receipt itself. The session-end hook ships unshipped receipts.
