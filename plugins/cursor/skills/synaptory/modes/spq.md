...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" next_action "$(pwd)" --cycle-id {CYCLE_ID}
```
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/advance_kernel.py" begin_dispatch "$(pwd)" {UNIT_ID} --role {ROLE_ABBREV}
python3 "${PLUGIN_ROOT}/hooks/lib/advance_kernel.py" advance "$(pwd)" {UNIT_ID} {TO_STATE} --receipt-path {RECEIPT_PATH}
```
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" hydrate_cycle "$(pwd)" --cycle-id {CYCLE_ID}
```
