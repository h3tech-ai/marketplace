...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" acceptance_status "$(pwd)"
```
...
3. **Move the stage.** Two edges, and which one is legal is decided in Step 3:
   ...
   python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" transition "$(pwd)" CYCLE
   ```
