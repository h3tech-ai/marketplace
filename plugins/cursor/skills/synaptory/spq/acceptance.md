...
> **MANDATORY: Spawn this agent via the `Task()` tool, do not run the security audit inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Task(subagent_type="general-purpose", description="CE release security audit", prompt=<self-contained prompt per the wrapper>)`: see `${PLUGIN_ROOT}/skills/_shared/backends/${CE_BACKEND}.md`. The CE writes its receipt to `${RECEIPTS_DIR}/ACCEPTANCE-{N}-ce.json` as its last action. Pass the resolved absolute path into the prompt; do not pass the literal `${RECEIPTS_DIR}`.
...
> **MANDATORY: Spawn this agent via the `Task()` tool, do not do the infrastructure work inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Task(subagent_type="general-purpose", description="PE production infrastructure", prompt=<self-contained prompt per the wrapper>)`: see `${PLUGIN_ROOT}/skills/_shared/backends/${PE_BACKEND}.md`. The PE writes its receipt to `${RECEIPTS_DIR}/ACCEPTANCE-{N}-pe.json` as its last action. Pass the resolved absolute path into the prompt; do not pass the literal `${RECEIPTS_DIR}`.
...
> **MANDATORY: Spawn this agent via the `Task()` tool, do not write the release documentation inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Task(subagent_type="general-purpose", description="TW release documentation", prompt=<self-contained prompt per the wrapper>)`: see `${PLUGIN_ROOT}/skills/_shared/backends/${TW_BACKEND}.md`. The TW writes its receipt to `${RECEIPTS_DIR}/ACCEPTANCE-{N}-tw.json` as its last action. Pass the resolved absolute path into the prompt; do not pass the literal `${RECEIPTS_DIR}`.
...
> **MANDATORY: Spawn this agent via the `Task()` tool, do not run the final review inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Task(subagent_type="general-purpose", description="CR release review", prompt=<self-contained prompt per the wrapper>)`: see `${PLUGIN_ROOT}/skills/_shared/backends/${CR_BACKEND}.md`. The CR writes its receipt to `${RECEIPTS_DIR}/ACCEPTANCE-{N}-cr.json` as its last action. Pass the resolved absolute path into the prompt; do not pass the literal `${RECEIPTS_DIR}`.
