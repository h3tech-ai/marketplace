...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" cut_work_unit "$(pwd)" {UNIT_ID} \
...
> **MANDATORY: Spawn this agent via the `Task()` tool.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost`. The dispatch looks like `Task(subagent_type="technical-writer", description="TW Cycle report", prompt=<self-contained prompt per the wrapper>)` -- see `${PLUGIN_ROOT}/skills/_shared/backends/${TW_BACKEND}.md`. Pass the resolved absolute path from `bind_receipt` into the prompt; do not pass the literal variable.
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" promote_cycle "$(pwd)" \
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" read_findings "$(pwd)"
```
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" close_cycle "$(pwd)" \
  --principal "<principal>" --rationale "<why>"
```
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" transition "$(pwd)" ACCEPTANCE
```
