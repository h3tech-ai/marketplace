...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" open_coordination_cycle "$(pwd)" \
  --goal "{RELEASE_GOAL}" \
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" publish_cross_cycle_event "$(pwd)" \
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" refresh_coordination "$(pwd)"
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" dep_status "$(pwd)"
```
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" release_readiness "$(pwd)"
```
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" revise_coordination_manifest "$(pwd)" \
...
```bash
python3 "${PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" clear_release "$(pwd)" \
  --cleared-by "{UPN}"
```
