...
Follow the Design Grooming Protocol at `${PLUGIN_ROOT}/skills/_shared/protocols/design-grooming.md`. For UI-surface projects the in-repo mockup baseline is **MANDATORY** and the Baseline Gate blocks on it; `design.enabled: false` disables only the optional Claude Design path, not the baseline.

Generate self-contained HTML/CSS mockups for the key screens using the design system reference under `${PLUGIN_ROOT}/skills/_shared/design-assets/` as the token source. Each page inlines its CSS (no external CDN/font/script fetches). Cover the landing screen, the primary user flow, and the core action at minimum. Write to `${design.mockups_dir}` (default `.synaptory/design/mockups/`): one `{screen}.html` per screen plus an `index.html` gallery.
...
4. Load `${PLUGIN_ROOT}/skills/synaptory/spq/commit.md` and open Slice 1.
