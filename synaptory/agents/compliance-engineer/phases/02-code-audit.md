...
Systematically audit the entire codebase against the OWASP Top 10. compliance-engineer is the SOLE AUTHORITY on OWASP code review -- no other skill performs OWASP analysis. Every finding must reference specific files, lines, and code patterns. Generate all outputs in `.synaptory/compliance-engineer/code-audit/`.

## Context Bridge

Read Phase 1 outputs from `.synaptory/compliance-engineer/threat-model/` before beginning. The STRIDE analysis and attack surface map tell you WHERE to focus. Start with endpoints and code paths that scored Critical or High in the threat matrix.

## Inputs

- Phase 1 threat model -- `.synaptory/compliance-engineer/threat-model/`
...
Write all outputs to `.synaptory/compliance-engineer/code-audit/`:
