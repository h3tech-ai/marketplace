...
CLI:
    ...
    python3 scrum_state_machine.py transition_story <project_dir> <story_id> <to_state> [--reason "..."]
...
def transition(
    ...
    if current == "INCEPTION" and to_state == "SPRINT_PLANNING":
        ...
        try:
            ...
            emit_project_inception_approved(project_slug, "orchestrator")
        except ImportError:
            pass
        ...
        try:
            ...
            emit_release_approved(release_id, "orchestrator")
        except ImportError:
            pass
...
def start_sprint(
    ...
    try:
        ...
        emit_spec_ready_opened([s["id"] for s in stories])
    except ImportError:
        pass
...
def transition_story(
    ...
    if to_state == "done" and from_state == "reviewing":
        _sprint_num = state.get("current_sprint", 1)
        _intensity = determine_dod_intensity(sprint_number=_sprint_num)
        ...
        if _block:
            ...
            # #116: route the legacy auto-promotion through awaiting_acceptance
            # when the toggle is on. Direct `awaiting_acceptance → done` (PO
    ...
    if to_state == "done" and from_state != "awaiting_acceptance":
        ...
        try:
            ...
            if dod.get("passed"):
                emit_evidence_dod_accepted(story_id, "orchestrator")
            else:
                failed_required = [
