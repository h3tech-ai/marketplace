...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" declare_sync_ready "$(pwd)" {SLICE_N} \
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" close_slice "$(pwd)" --proceed-to COMMIT
```
