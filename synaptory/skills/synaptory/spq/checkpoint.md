...
> **MANDATORY: Spawn this agent via the `Agent()` tool, do not write the Slice reports inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Agent(subagent_type="general-purpose", description="TW Slice {N} report", prompt=<self-contained prompt per the wrapper>)`: see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/backends/${TW_BACKEND}.md`. The TW writes its receipt to `.synaptory/.orchestrator/receipts/CHECKPOINT-{N}-tw.json` as its last action.
...
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" accept_story "$(pwd)" "{WU-ID}" \
  --accepted-by "{PO_EMAIL_OR_ROLE}"

# Reject → reason class + free-form feedback, appended to rejection_feedback
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" reject_story "$(pwd)" "{WU-ID}" \
...
> **MANDATORY: Spawn this agent via the `Agent()` tool, do not process the feedback inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Agent(subagent_type="general-purpose", description="PO Slice {N} backlog update", prompt=<self-contained prompt per the wrapper>)`: see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/backends/${PO_BACKEND}.md`. The PO writes its receipt to `.synaptory/.orchestrator/receipts/CHECKPOINT-{N}-po.json` as its last action.
...
```
{IF COMMIT}:     → Read("${CLAUDE_PLUGIN_ROOT}/skills/synaptory/spq/commit.md")
{IF ACCEPTANCE}: → Read("${CLAUDE_PLUGIN_ROOT}/skills/synaptory/spq/acceptance.md")
```
