...
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/sync_barrier.py" declare-ready "$(pwd)" {N} \
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" declare_sync_ready "$(pwd)" {N} \
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/sync_barrier.py" collect "$(pwd)" {N}
```
...
```bash
# on sync/slice-{N}
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/sync_barrier.py" evaluate "$(pwd)" {N}
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" clear_sync "$(pwd)" {N} \
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/spq_state_machine.py" cut_work_unit "$(pwd)" {WU-ID} \
...
> **MANDATORY: Spawn this agent via the `Agent()` tool, do not run the integration pass inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Agent(subagent_type="general-purpose", description="QE Slice {N} integration pass", prompt=<self-contained prompt per the wrapper>)`: see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/backends/${QE_BACKEND}.md`. The QE writes its receipt to `.synaptory/.orchestrator/receipts/SYNC-{N}-qe.json` as its last action.
...
> **MANDATORY: Spawn this agent via the `Agent()` tool, do not resolve the conflict inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Agent(subagent_type="general-purpose", description="SE conflict resolution {WU-ID} for Slice {N}", prompt=<self-contained prompt per the wrapper>)`: see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/backends/${SE_BACKEND}.md`. The SE writes its receipt to `.synaptory/.orchestrator/receipts/{WU-ID}-se.json` as its last action.
...
> **MANDATORY: Spawn this agent via the `Agent()` tool, do not adjudicate the drift inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Agent(subagent_type="general-purpose", description="SA shared-digest adjudication Slice {N}", prompt=<self-contained prompt per the wrapper>)`: see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/backends/${SA_BACKEND}.md`. The SA writes its receipt to `.synaptory/.orchestrator/receipts/SYNC-{N}-sa.json` as its last action.
...
> **MANDATORY: Spawn this agent via the `Agent()` tool, do not run the security review inline.** Inline execution skips the SubagentStop hook, so no receipt is written and the work never reaches `/cost` or `/quality`. The dispatch must look like `Agent(subagent_type="general-purpose", description="CE integrated auth/data surface Slice {N}", prompt=<self-contained prompt per the wrapper>)`: see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/backends/${CE_BACKEND}.md`. The CE writes its receipt to `.synaptory/.orchestrator/receipts/SYNC-{N}-ce.json` as its last action.
