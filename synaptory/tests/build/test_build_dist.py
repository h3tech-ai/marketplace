...
def fresh_build(repo_root: Path, tmp_path_factory) -> Path:
    ...
    plugin_dist = dist_root / "synaptory"
...
def test_marketplace_json_catalog_present(fresh_build: Path):
    ...
    catalog = fresh_build.parent / ".claude-plugin" / "marketplace.json"
    ...
    # object form `{"type":"local","path":"./synaptory"}` was rejected by
    ...
    assert entry["source"] == "./synaptory", (
        f"`source` must be './synaptory', got {entry['source']!r}"
    )
...
def test_codex_package_and_catalog_present(fresh_build: Path, repo_root: Path):
    ...
    marketplace_root = fresh_build.parent
    codex_plugin = marketplace_root / "plugins" / "synaptory"
...
def test_marketplace_json_schema_compatible(fresh_build: Path):
    ...
    """
    catalog_path = fresh_build.parent / ".claude-plugin" / "marketplace.json"
